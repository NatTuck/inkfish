defmodule InkfishWeb.ApiV1.SubControllerTest do
  ...
  describe "index" do
    test "lists all subs", %{conn: conn} do
      conn = get(conn, ~p"/api/api_v1/subs")
  ...
  describe "create sub" do
    test "renders sub when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/api/api_v1/subs", sub: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, ~p"/api/api_v1/subs/#{id}")
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/api_v1/subs", sub: @invalid_attrs)
  ...
  describe "update sub" do
    ...
    test "renders sub when data is valid", %{conn: conn, sub: %Sub{id: id} = sub} do
      conn = put(conn, ~p"/api/api_v1/subs/#{sub}", sub: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, sub: sub} do
      conn = put(conn, ~p"/api/api_v1/subs/#{sub}", sub: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete sub" do
    ...
    test "deletes chosen sub", %{conn: conn, sub: sub} do
      conn = delete(conn, ~p"/api/api_v1/subs/#{sub}")
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, ~p"/api/api_v1/subs/#{sub}")
      end
    end
  end
